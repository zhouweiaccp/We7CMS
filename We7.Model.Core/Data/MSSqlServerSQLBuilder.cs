...
{
    public class MSSqlServerSQLBuilder : ISQLBuilder
    {
        /// <summary>
        /// 添加数据
        /// </summary>
        ...
        /// 分页列表
        /// </summary>
        const string SQLPAGELIST = @"SELECT * FROM (SELECT TOP {4} * FROM (SELECT TOP {0} * FROM {1} {2} ORDER BY {3}) AS TB_1 ORDER BY {5}) AS TB_2 ORDER BY {3}";
        /// <summary>
        /// 前多少条记录
        /// </summary>
        const string SQLTOPLIST = @"SELECT TOP {0} * FROM {1} {2} ORDER BY {3}";
        /// <summary>
        /// 列表数据
        /// </summary>
        ...
        public string BuildInsertSQL(PanelContext data, System.Data.IDataParameterCollection parameters)
        {
            ...
            foreach (DataField field in data.Row)
            {
                ...
                {
                    SqlParameter param = new SqlParameter(field.Column.Name,SqlDbType.DateTime);
                    param.Value = field.Value;
                    parameters.Add(param);
                }
                else
                {
                    parameters.Add(new SqlParameter(paramname, field.Value));
                }
            }
        ...
        public string BuildUpdateSQL(PanelContext data, System.Data.IDataParameterCollection parameters)
        {
            ...
            foreach (DataField field in data.Row)
            {
                ...
                {
                    parameters.Add(new SqlParameter(field.Column.Name, field.Value));
                }
            }
        ...
        StringBuilder BuilderWhereSqlByDataKey(PanelContext data, IDataParameterCollection parameters, string prefix)
        {
            ...
            for (int i = 0; i < data.Panel.Context.DataKeys.Length; i++)
            {
                ...
                {
                    SqlParameter param = new SqlParameter(column.Name,SqlDbType.DateTime);
                    param.Value = data.DataKey[fieldname];
                    parameters.Add(param);
                }
                else
                {
                    parameters.Add(new SqlParameter(paramname, data.DataKey[fieldname]));
                }
            }
            return sqlwehre;
        }
        ...
        StringBuilder BuildWhereSql(PanelContext data, IDataParameterCollection parameters, string prefix)
        {
            ...
            for (int i = 0; i < data.QueryFields.Count; i++)
            {
                ...
                parameters.Add(new SqlParameter(paramname, qf.Value));
            }
            return sqlwehre;
        }
    }
