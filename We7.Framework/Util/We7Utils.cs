...
namespace We7.Framework.Util
{
    ...
    public class We7Utils
    {
        ...
        public static void ResponseText(string text, string filename, string filetype)
        {
        ...
        public static string RemoveUnsafeHtml(string content)
        {
            ...
            //TODO::thj修改。添加了必须是在<>中的验证
            content = Regex.Replace(content, @"(?<=<\s*)(script|frame|form|meta|behavior|style)([\s|:|>])+", "$1.$2", RegexOptions.IgnoreCase);
            return content;
        }
        ...
        public static Control FindControl(string id, Control container)
        {
            if (String.IsNullOrEmpty(id))
                throw new ArgumentNullException("查找的控件ID不能为空");

            if (container == null)
                throw new ArgumentNullException("查找容器不能为空");

            Control ctr=container.FindControl(id);
            if (ctr == null)
            {
                foreach (Control c in container.Controls)
                {
                    ctr =FindControl(id, c);
        ...
        public static object Invoke(string fullTypeName, string method, object[] args)
        {
            object instance = Utils.CreateInstance(fullTypeName);
            if (instance == null)
                throw new Exception(fullTypeName + "类型不存在");
            Type type = instance.GetType();
            return type.InvokeMember(method, BindingFlags.InvokeMethod, null, instance, args);
        }

        public static T GetCacheInstance<T>(string cacheKey, CreateInstanceHandler<T> handler) where T : class
        {
            T result = AppCtx.Cache.RetrieveObject<T>(cacheKey);
            if (result == null && handler != null)
            {
                result = handler();
                AppCtx.Cache.AddObject(cacheKey, result);
            }
            return result;
        }

        public static T GetCacheInstance<T>(string cacheKey, CreateInstanceHandler<T> handler,int timeout) where T : class
        {
            T result = AppCtx.Cache.RetrieveObject<T>(cacheKey);
            if (result == null && handler != null)
            {
                result = handler();
                AppCtx.Cache.AddObject(cacheKey, result,timeout);
            }
            return result;
        }

        public static T GetCacheInstance<T>(string cacheKey, CreateInstanceHandler2<T> handler,params string[] filePath) where T : class
        {
            T result = AppCtx.Cache.RetrieveObject<T>(cacheKey);
            if (result == null && handler != null)
            {
                result = handler(filePath);
                AppCtx.Cache.AddObjectWithFileChange(cacheKey, result, filePath);
            }
            return result;
        }
        ...
        public static string CNToUnicode(string s)
        {
            ...
            if (!String.IsNullOrEmpty(s))
            {
                foreach (char c in s)
                {
                    if (c > 255)
                    {
                        sb.AppendFormat("\\u{0}", ((int)c).ToString("x"));
                    }
                    else
                    {
                        sb.Append(c);
        ...
        public static string UnicodeToCN(string s)
        {
            ...
            if (!String.IsNullOrEmpty(s))
            {
                string[] ss=s.Replace("\\", "").Split('u');
                foreach (string c in ss)
                {
                    sb.Append((char)int.Parse(c, NumberStyles.HexNumber));
                }
            }
            return sb.ToString();
        }
    ...
    public delegate TResult CreateInstanceHandler<TResult>();
    public delegate TResult CreateInstanceHandler2<TResult>(params string[] filePath);
