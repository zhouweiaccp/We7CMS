...
function FCK_ContextMenu_GetListener( listenerName )
{
	switch ( listenerName )
	{
		...
		case 'TextField' :
			...
			AddItems : function( menu, tag, tagName )
			{
				if ( tagName == 'INPUT' && ( tag.type == 'text' || tag.type == 'password' ) && tag.className!='_wf_date')
				{
					menu.AddSeparator() ;
	   ...
	   case 'We7_Subtemplate':
			return {
			AddItems: function(A,B,C)
			{
//			    alert(A+";"+C+";"+B);
//			    debugger;
                var node =B;
                if(node!=null)
                {
                    if(C=='IMG' && node!=null &&(node.tag=='wet' || node.className=='wet'))
			        {
			            A.AddSeparator();
			            A.AddItem('We7_Subtemplate',FCKLang.We7SubtemplateProp,6);
			        }
			        
                    if(!(node.tagName=="DIV" && ( node.tag=='wet' || node.className=='wet')))
                        node=node.parentNode;
			        if(C=='DIV' && node!=null &&(node.tag=='wet' || node.className=='wet'))
			        {
			            A.AddSeparator();
			            A.AddItem('We7_Subtemplate',FCKLang.We7SubtemplateProp,6);
			        }
	  ...
	  case 'We7_Control':
			...
			AddItems: function(A,B,C)
			{
			    if(C=='DIV' &&( B.className=='wec' || B.tag=='wec' ))
			    {
			    A.AddSeparator();
			    A.AddItem('We7ControlProp',FCKLang.We7ControlProp,6);
			    }
			}};
			
	  case 'We7_Control_New':
			return {
			AddItems: function( menu, tag, tagName )
			{
			    if(tagName=='IMG' &&( tag.className=='wec' || tag.tag=='wec' ))
			    {
			    menu.AddSeparator();
			    menu.AddItem('We7ControlProp_New',FCKLang.We7ControlProp,6);
			    }
    			
			    if(tagName=='DIV' &&( tag.className=='wec' || tag.tag=='wec' ))
			    {
			    menu.AddSeparator();
			    menu.AddItem('We7ControlProp_New',FCKLang.We7ControlProp,6);
			    }
